...
add_para(doc,
    ...
    '我們選擇這個題目，因為偏鄉與資源有限家庭常缺乏可長期使用的動作觀察工具，'
...
add_para(doc,
    '情境：使用者每天早上完成坐站訓練後，打開手機上的 CareLoop，'
    '讓系統透過前鏡頭觀察她做五次坐站的動作，分析結果自動儲存在手機本機。'
    '下午家人打來關心時，可以問「今天狀況怎樣」，'
...
gen_items = [
    ...
    '偏斜角度：基礎 5° + z-score × 3.2°，加個體隨機變異（SD ≈ 4°），上限 45°；決策閾值採居家攝影機容錯後的 20° / 35°',
...
risks = [
    ...
    ('網路不穩定', 'MediaPipe WASM 與模型已本機化；核心拍攝測試不依賴外部 CDN'),
...
checklist = [
    ...
    ('文案合規', [
    '所有頁面中不出現「醫療診斷」「高風險判定」「法規認可」等過度宣稱',
        '所有宣稱均有具體文獻依據',
        '誠實聲明與侷限性揭露在主要頁面可見',
